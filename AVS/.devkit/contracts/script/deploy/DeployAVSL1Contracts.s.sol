...
contract DeployAVSL1Contracts is Script {
    function run(
        ...
        RewardFlowAVSRegistrar rewardFlowRegistrarImpl = new RewardFlowAVSRegistrar(
            IAllocationManager(allocationManager),
            IKeyRegistrar(keyRegistrar),
            IPermissionController(permissionController)
        );
        console.log("RewardFlowAVSRegistrar implementation deployed to:", address(rewardFlowRegistrarImpl));

        // Deploy proxy with initialization
        TransparentUpgradeableProxy proxy = new TransparentUpgradeableProxy(
            address(rewardFlowRegistrarImpl),
            address(proxyAdmin),
            abi.encodeWithSelector(
                RewardFlowAVSRegistrar.initialize.selector, avs, vm.addr(deployerPrivateKey), initialConfig
            )
        );
        console.log("RewardFlowAVSRegistrar proxy deployed to:", address(proxy));
        ...
        for (uint256 i = 0; i < aggregatorWhitelistedOperators.length; i++) {
            RewardFlowAVSRegistrar(address(proxy)).addOperatorToAllowlist(
                aggregatorOperatorSet, aggregatorWhitelistedOperators[i]
            );
        }

        // Transfer ownership of the proxy to the avs
        RewardFlowAVSRegistrar(address(proxy)).transferOwnership(avs);
        ...
        _writeOutputToJson(environment, address(proxy), address(rewardFlowRegistrarImpl), address(proxyAdmin));
    }

    function _writeOutputToJson(
        string memory environment,
        address rewardFlowRegistrarProxy,
        address rewardFlowRegistrarImpl,
        address proxyAdmin
    ) internal {
        // Add the addresses object
        string memory addresses = "addresses";
        vm.serializeAddress(addresses, "rewardFlowAVSRegistrar", rewardFlowRegistrarProxy);
        vm.serializeAddress(addresses, "rewardFlowAVSRegistrarImpl", rewardFlowRegistrarImpl);
